...
config.py
=========
Configuración de ESTA carpeta / divisa. Los tres scripts (mt5_export_h4.py,
auto_channels.py, events_to_excel.py) leen automáticamente estos valores
como valores por defecto — así puedes correrlos SIN escribir ningún parámetro.

CÓMO USAR UNA DIVISA NUEVA:
  1. Copia esta carpeta completa (la "carpeta base").
  2. Renombra la copia con el nombre de la divisa, ej: "EURUSD", "USDJPY", "EURGBP".
  3. Ya está — el símbolo se detecta solo a partir del nombre de la carpeta.
     No hay que tocar nada más, a menos que quieras cambiar la fecha o algún parámetro.
"""
import os

# ── Divisa: se toma automáticamente del nombre de esta carpeta ──
SYMBOL = os.path.basename(os.path.dirname(os.path.abspath(__file__)))
